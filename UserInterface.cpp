...
void UserInterface::RunMenu(const WeatherLog& log)
{
 ...
 void UserInterface::Choice_One(const WeatherLog& log)
    ...
    void UserInterface::Choice_Three(const WeatherLog& log)
    {
        ...
        {
            Vector<WeatherRecord> rows = WeatherAnalytics::FilterMonthYear(log, m, year); // Filter records

            double total; // Store solar total result
            cout << MonthName(m) << ": ";  // Print month label

            if (!WeatherAnalytics::SolarTotalKWhm2(rows, total)) // Compute solar totals (returns false if none)
            {
                cout << "No Data\n";  // Print required "No Data"
            }
            else
            {
                cout << fixed << setprecision(1) << total << " kWh/m^2\n"; // print formatted output
            }
        }
    }
    ...
    void UserInterface::Choice_Four(const WeatherLog& log)
    {
        ...
        for (int m = 1; m <= 12; m++) // Loop through months
        {
            ...
            if (!wroteAnyMonth) // If this is the first month with data
            {
                ...
                cout << "Month,Average Wind Speed(stdev),Average Ambient Temperature(stdev),Solar Radiation\n";
            ...
            if (wind.Size() > 0) // Only compute if wind values exist
            {
                ...
                cout << fixed << setprecision(1) << wMean << "(" << wSd << ")"; // Output wind field
                out  << fixed << setprecision(1) << wMean << "(" << wSd << ")"; // Write wind field
            }
            ...
            if (temp.Size() > 0) // Only compute if temperature values exist
            {
                ...
                cout << fixed << setprecision(1) << tMean << "(" << tSd << ")"; // Output temp field
                out  << fixed << setprecision(1) << tMean << "(" << tSd << ")"; // Write temp field
            }
            cout << ","; // Output comma separator
            out  << ","; // Write comma separator
